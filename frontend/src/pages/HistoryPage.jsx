...
export default function HistoryPage() {
  ...
  const [selectedProjectId, setSelectedProjectId] = useState(id && id !== 'undefined' ? id : '');
  ...
  const loadProjectsAndHistory = async () => {
    try {
      ...
      const userProjects = await projectService.getProjects().catch(() => []);
      setProjects(userProjects || []);

      let targetId = id && id !== 'undefined' ? id : selectedProjectId;
      if (!targetId && userProjects && userProjects.length > 0) {
        targetId = userProjects[0].id;
      }

      if (targetId) {
        setSelectedProjectId(targetId);
        const historyRuns = await projectService.getAnalysisRuns(targetId);
        setRuns(historyRuns || []);
      } else {
        setRuns([]);
      }
  ...
  const handleProjectSelect = async (projId) => {
    setSelectedProjectId(projId);
    try {
      setLoading(true);
      setError(null);
      const data = await projectService.getAnalysisRuns(projId);
      setRuns(data || []);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to load history for selected project.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReanalyze = async () => {
    ...
    try {
      setReanalyzing(true);
      await projectService.triggerAnalysis(selectedProjectId);
      await loadProjectsAndHistory();
    } catch (err) {
  ...
  const handleOpenDetails = async (runId) => {
    if (!selectedProjectId) return;
    try {
      const details = await projectService.getAnalysisRunDetails(selectedProjectId, runId);
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col gap-4 border-b border-[#1e1e3a] pb-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {projects.length > 0 && (
            <div className="flex items-center gap-2 bg-[#0e0e24] px-3 py-1.5 rounded-lg border border-[#1e1e3a]">
              ...
              >
                {projects.map(p => (
                  <option key={p.id} value={p.id} className="bg-[#0c0c1e] text-slate-200">
                    {p.name || p.repo_url}
                  </option>
                ))}
              </select>
          ...
          {selectedProjectId && (
            <>
              <Link
                to={`/projects/${selectedProjectId}/compare`}
      ...
      {error ? (
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-6 text-center text-red-300">
          ...
          <button
            onClick={loadProjectsAndHistory}
        ...
        <div className="card py-16 text-center text-slate-400 space-y-3 border-dashed border-[#1e1e3a]">
          <span className="text-4xl">📁</span>
          <p className="text-base font-bold text-slate-200">No Repository Projects Available</p>
          <p className="text-xs text-slate-400">Add a GitHub repository project from the Dashboard to start tracking analysis history.</p>
          <div className="pt-2">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-base font-bold text-slate-100 flex items-center gap-2">
                  ...
                  {activeProject && (
                    <span className="text-xs text-indigo-400 font-normal">({activeProject.name})</span>
                  )}
                </h2>
          ...
          <div className="card space-y-4">
            ...
            {runs.length === 0 ? (
              <div className="py-8 text-center text-slate-500">
                No analysis runs recorded yet for this project. Click "Re-analyze Repo" to trigger an analysis run.
              </div>
            ) : (
